...
TrimerGen is a molecular utility program that generates trimers (assuming rigid monomers). The trimers are
...
with the coordinates: $( R_{\rm COM}^{\rm AB}, R_{\rm COM}^{\rm BC}, R_{\rm COM}^{\rm AC}, \beta_A, \gamma_A, \alpha_B, \beta_B, \gamma_B, \alpha_C, \beta_C, \gamma_C )$. Note: $\alpha_A$ is hardcoded and set to 0. $\theta$ represents the angle $\angle {\rm BAC}$ in the triangle formed by the three monomer COMs. It is determined from the three COM separations via the law of cosines:
